...
function createOperationReport(rows, tables, scoreRecords, sinceTimestamp = startOfTodayTimestamp(), now = Date.now()) {
  ...
  const completedRows = (rows || []).filter((row) => Number(row.acknowledged?.extraCoalFlip || 0) >= sinceTimestamp);
  ...
  return {
    sinceTimestamp,
    endedAt: now,
    hookahCount: completedRows.length,
...
function HookahTimerAppInner() {
  ...
  function startShift() {
    const hasExistingRows = rows.length > 0 || refillReminders.length > 0;
    if (hasExistingRows) {
      const ok = window.confirm("새 영업을 시작하면 남아있는 타이머/알림을 정리합니다. 시작할까요?");
      if (!ok) return;
    }
    ...
    setNotificationStatus(`영업 시작 · ${formatDateTime(new Date(startedAt))}`);
  }
      ...
      <div className="mx-auto max-w-7xl space-y-3 md:space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className={`rounded-[1.75rem] border px-5 py-5 text-lg font-black text-white shadow-2xl transition ${shiftActive ? "border-red-500/50 bg-gradient-to-br from-red-700/90 to-red-950/90 shadow-red-950/40 hover:from-red-600 hover:to-red-900" : "border-emerald-400/45 bg-gradient-to-br from-emerald-500/80 to-emerald-950/90 shadow-emerald-950/30 hover:from-emerald-500 hover:to-emerald-900"}`}
          >
            {shiftActive ? "퇴근!" : "영업 시작"}
            <span className={`mt-1 block text-xs font-bold ${shiftActive ? "text-red-100/65" : "text-emerald-100/75"}`}>
              {shiftActive ? "오늘 영업 리포트 보기" : "오늘 기록 새로 시작"}
            </span>
          </button>
          <button
            type="button"
            onClick={() => setShowScoreSummary(true)}
            className="rounded-[1.75rem] border border-amber-400/35 bg-gradient-to-br from-amber-500/25 to-black/65 px-5 py-5 text-lg font-black text-white shadow-2xl shadow-black/30 transition hover:border-amber-300/60 hover:from-amber-500/35"
          >
            점수 확인
