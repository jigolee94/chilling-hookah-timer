...
function prepareStoredRows(storedRows, tables, fallbackSettings = defaultSettings) {
  ...
  return storedRows.map((row) => {
    ...
    return {
      ...
      urgentDismissed: row.urgentDismissed || {},
...
function HookahTimerAppInner() {
  ...
  const [mainDeviceClaimedAtMs, setMainDeviceClaimedAtMs] = useState(0);
  ...
  useEffect(() => {
    try {
      ...
      if (data) {
        ...
        setMainDeviceClaimedAtMs(Number(data.mainDeviceClaimedAtMs || 0));
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ settings, tables, fixtures, rows, refillReminders, adminId, adminPin, mainDeviceId, mainDeviceClaimedAtMs, shift, selectedTableId, selectedLayoutTarget, layoutPresets, selectedPresetId }));
    } catch (error) {
      console.warn("Failed to save data", error);
    }
  }, [storageReady, settings, tables, fixtures, rows, refillReminders, adminId, adminPin, mainDeviceId, mainDeviceClaimedAtMs, shift, selectedTableId, selectedLayoutTarget, layoutPresets, selectedPresetId]);
  ...
  function tableSummary(tableId) {
    ...
    const urgentDue = Boolean(next && !next.row.urgentDismissed?.[next.nextTask.key] && nextSeconds !== null && nextSeconds > 0 && nextSeconds <= 60);
    const critical = urgentDue;
    const soon = urgentDue;
    ...
    const sync = async () => {
      try {
        const result = await syncStoreSnapshot({
          ...
          isMainDevice: true,
          mainDeviceClaimedAtMs,
          tables: buildAdminSyncTables(Date.now()),
        });
        if (!active) return;
        if (result?.skipped) {
          if (result.reason === "outdated-main-device") {
            setMainDeviceId("");
            setMainDeviceClaimedAtMs(0);
            setLastAdminSyncAt(null);
            setAdminSyncError("다른 기기가 더 최근 메인기기로 지정되어 이 기기의 전송을 중지했습니다.");
            return;
          }
          setAdminSyncError(result.reason === "not-main-device" ? "메인기기 지정 필요" : "Firebase 환경값이 없어 관리자 앱으로 전송하지 못했습니다.");
          return;
        }
        setAdminSyncError("");
  ...
  }, [storageReady, isMainSyncDevice, selectedPresetId, selectedPreset?.name, settings, tables, rows, refillReminders, deviceId, mainDeviceClaimedAtMs]);

  function designateMainDevice() {
    const claimedAt = Date.now();
    setMainDeviceId(deviceId);
    setMainDeviceClaimedAtMs(claimedAt);
    setLastAdminSyncAt(null);
    setAdminSyncError("");
    setNotificationStatus("이 기기를 메인기기로 지정했습니다. 이전 관리자 앱 데이터는 지우고 이 기기의 테이블 상태만 보냅니다.");
  }
      ...
      prev.map((row) =>
        row.id === id
          ? {
              ...row,
              ...patch,
              ...(timingChanged ? { alarmed: {}, alarmedAt: {}, alarmRepeatAt: {}, urgentAlarmed: {}, urgentDismissed: {}, coverAlarmed: {}, coverDismissed: {}, acknowledged: {}, timeAdjustments: {}, confirmationHistory: {} } : {}),
            }
          : row
      )
    );
  ...
  function clearTaskStateFrom(row, taskKey, options = {}) {
    ...
    const nextUrgentDismissed = { ...(row.urgentDismissed || {}) };
    ...
    TASK_ORDER.slice(startIndex).forEach((key) => {
      ...
      if (!options.preserveDismissedAlerts) delete nextUrgentDismissed[key];
      delete nextCoverAlarmed[key];
      if (!options.preserveDismissedAlerts) delete nextCoverDismissed[key];
      delete nextAcknowledged[key];
      delete nextConfirmationHistory[key];
    });

    return { alarmed: nextAlarmed, alarmedAt: nextAlarmedAt, alarmRepeatAt: nextAlarmRepeatAt, urgentAlarmed: nextUrgentAlarmed, urgentDismissed: nextUrgentDismissed, coverAlarmed: nextCoverAlarmed, coverDismissed: nextCoverDismissed, acknowledged: nextAcknowledged, confirmationHistory: nextConfirmationHistory };
  }
  ...
  function adjustTaskTime(rowId, taskKey, minutes) {
    ...
    setRows((prev) =>
      prev.map((row) => {
        ...
        const taskState = clearTaskStateFrom(row, taskKey, { preserveDismissedAlerts: true });
        return {
          ...row,
          ...taskState,
          timeAdjustments: {
  ...
  function dismissUrgentAlert(event, rowId, taskKey) {
    ...
    setRows((prev) =>
      prev.map((row) =>
        row.id === rowId
          ? {
              ...row,
              urgentDismissed: { ...(row.urgentDismissed || {}), [taskKey]: true },
            }
          : row
      )
    );
    ...
    setRows((prev) =>
      prev.map((row) => {
        ...
        TASK_ORDER.slice(startIndex).forEach((key) => {
          ...
          delete nextAlarmRepeatAt[key];
          delete nextUrgentAlarmed[key];
          delete nextUrgentDismissed[key];
          delete nextCoverAlarmed[key];
        ...
        const basePatch = {
          ...
          urgentDismissed: nextUrgentDismissed,
  ...
  function renderTimerCard(item, options = {}) {
    ...
    const displayUrgentDue = soon && !overdue && !row.urgentDismissed?.[nextTask.key];
    ...
    return (
      ...
      >
        ...
        {options.inPopup && displayUrgentDue && (
          <button
            type="button"
            onPointerDown={(event) => {
              event.stopPropagation();
            ...
            onClick={(event) => dismissUrgentAlert(event, row.id, nextTask.key)}
            className="table-state-overlay table-urgent-overlay"
            aria-label={`${table?.name || "테이블"} ${nextTask.label} 임박 알림 닫기`}
          >
            <span>임박</span>
          </button>
        )}
            ...
            {tables.map((table) => {
              ...
              return (
                <div
                  ...
                  className="absolute w-[134px] md:w-[158px]"
                ...
                >
                  <div className={`relative min-h-[112px] rounded-3xl border p-3 text-left transition-all ${tableCardStateClass}`}>
                    <button onClick={(event) => selectTableOnly(event, table.id)} className="w-full text-left pr-9">
                      ...
                      <div className={`mt-1 min-h-[44px] ${summary.refillDue || summary.coverDue ? "text-emerald-100/75" : summary.critical || summary.soon ? "text-amber-100/85" : "text-red-100/55"}`}>
                        {summary.next ? (
                          <>
                            <div className="truncate text-[15px] font-black leading-5 text-white">{summary.next.nextTask.label}</div>
                            <div className={`mt-1 whitespace-nowrap text-[12px] font-bold leading-none tabular-nums ${summary.overdue || summary.critical ? "text-white" : ""}`}>{formatTime(summary.next.nextTask.time)} · {statusLabel(summary.next.nextTask.time)}</div>
                          </>
                        ) : (
                          <div className="text-xs leading-4 text-red-100/25">비어있음</div>
                        )}
                      </div>
                    </button>
                    ...
                    {summary.critical && !summary.overdue && summary.next && !timerMoveMode && (
                      <button
                        ...
                        onClick={(event) => dismissUrgentAlert(event, summary.next.row.id, summary.next.nextTask.key)}
                        className="table-state-overlay table-urgent-overlay"
                        aria-label={`${table.name} ${summary.next.nextTask.label} 임박 알림 닫기`}
                      >
                        <span>임박</span>
                      </button>
                    )}
